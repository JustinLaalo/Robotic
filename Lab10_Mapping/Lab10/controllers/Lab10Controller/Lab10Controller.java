...
public class Lab10Controller {
  ...
  static void addSensorReadingsToMap(MapperApp mapper, Field translationField, double x, double y, double a){
    ...
    a = Math.toRadians(a);
    for(int i = 0; i < rangeSensors.length; i++){
      double d = rangeSensors[i].getValue() * 100;
      if(d<10){
         xObj = x + xOffsets[i] * Math.cos(a) - yOffsets[i] * Math.sin(a) + d * Math.cos(a + Math.toRadians(aOffsets[i]));
         yObj = y + yOffsets[i] * Math.cos(a) + xOffsets[i] * Math.sin(a) + d * Math.sin(a + Math.toRadians(aOffsets[i]));
  ...
  public static void main(String[] args) {
    ...
    while (robot.step(timeStep) != -1) {
      ...
      if(desiredTurn != SPIN_RIGHT || desiredTurn != SPIN_LEFT){
        addSensorReadingsToMap(mapper, translationField, X, Y, getCompassReadingInDegrees(compass));
      }
