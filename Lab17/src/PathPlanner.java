...
public class PathPlanner {
  ...
  public ArrayList<Point> computeSupportPointsFrom(ArrayList<Obstacle> obstacles, int x, int y) {
    ...
    for (Obstacle ob: obstacles) {
      ...
      for (int i=0; i<ob.size(); i++) {
        ...
        Point bvi = ob.getVertex((i-1+ob.size())%ob.size());
        Point avi = ob.getVertex((i+1)%ob.size());
  ...
  public void computeVisibilityGraph() {
    ArrayList<Obstacle> obstacles = vmap.getConvexObstacles();
    ...
    supports = computeSupportPointsFrom(obstacles, start.x, start.y);
    ...
    for (Node n: visibilityGraph.getNodes()) {
      if ((n == s) || (n==e))
        continue;
      supports = computeSupportPointsFrom(obstacles, n.getLocation().x, n.getLocation().y);
      for (Point p: supports) {
        Node m = visibilityGraph.node(p.x, p.y);
        if (m != null)
  ...
  private void runDijkstrasAlgorithmFrom(Node s) {
    ...
    for(Node v: visibilityGraph.getNodes()){
    ...
    while(!q.isEmpty()){
      Node v = q.poll();
      for(Edge e: visibilityGraph.getEdges()){
        if(e.getStartNode() == v || e.getEndNode() == v){
          Node u = e.otherEndFrom(v);
          if(u.getDistance() > (v.getDistance()+ e.length())){
            q.remove(u);
            u.setPrevious(v);
            u.setDistance(v.getDistance()+e.length());
            q.add(u);
          }
        }
      }
